...
void RpcProvider::NotifyService(google::protobuf::Service *service)
{
    ...
    // 输出 m_serviceMap 的内容
    std::cout << "Current service map contents:" << std::endl;
    for (const auto &entry : m_serviceMap)
    {
        LOG_INFO("service_name:%s", entry.first.c_str());

        for (const auto &method_entry : entry.second.m_methodMap)
        {
            LOG_INFO("method_name :%s", method_entry.first.c_str());
        }
    }
};
...
void RpcProvider::Run()
{
    ...
    // 把当前 rpc节点上要发布的服务全部注册到zk上面，让rpc client可以从zk上发现服务
...
/*
    在框架内部, RpcProvider 和 RpcConsumer 协商好通信用的 protobuf 数据类型
    service_name method_name args 定义 proto的message类型, 进行数据头的序列化和反序列化.
                                 service_name method_name args_size
    header_size(4字节) + header_str + args_str
*/

// 已建立连接的读写事件回调. 如果远程有一个 rpc 服务的调用请求, 该方法就会响应.
                            ...
                            muduo::Timestamp)
{
    ...
    // std::cout << "Received data size: " << recv_buf.size() << std::endl;
    ...
    {
        // 数据头反序列化失败.
        std::cout << "rcp_header_str : " << rpc_header_str << "parse error!" << std::endl;
        return;
    }
    ...
    {
        std::cout << "Service not found in m_serviceMap: " << service_name << std::endl;
        return;
    }
    ...
    {
        std::cout << "Method not found in service: " << service_name << ", method: " << method_name << std::endl;
        return;
    }
    ...
    // 给下面 method 方法的调用, 绑定一个 closure 的回调函数.
    ...
    // 在框架上根据远程 rpc 请求, 调用当前rpc 节点上发布的方法.
